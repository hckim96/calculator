...
export const HomeScreen = ({navigation}) => {
  ...
  const [user, setUser] = useState({
    name: 'name',
    militaryClass: 'mClass',
    beginDay: new Date('January 30, 2020'),
    dischargeDay: 'dDay',
    passedDays: 'pDay',
    leftDays: 'lDay',
  });
  ...
  const [passed_pct, setPassed_pct] = useState(
    ((now - beginDay) / (dischargeDay - beginDay)) * 100,
  );
  const dday = [
    {
      dayName: '전체 복무일',
  ...
  function getWorkingDay(monthIndex) {
    let total =
      new Date(2020, monthIndex + 1, 1) - new Date(2020, monthIndex, 1);
  ...
  function getTotalPay() {
    ...
    let totalPay =
      BASE_PAY[militaryClass] +
      workingDays * (foodExpenses + transportationExpenses);
    return (
      <View>
        <Text style={{textAlign: 'center', fontSize: 20, fontWeight: 'bold'}}>
          {`${parseInt(totalPay / 1000)},${
            parseInt(totalPay) % 1000 == 0 ? '000' : parseInt(totalPay) % 1000
          }`}
        </Text>
        <Text style={{fontSize: 15, fontWeight: 'bold', textAlign: 'center'}}>
          {`(${BASE_PAY[militaryClass]} + ${workingDays} * ${
            foodExpenses + transportationExpenses
          })`}
        </Text>
      </View>
    );
